...
function CCCH_MODE_REQ_handle(phyconnect,gsmphy,dfe,det,dec,tpu)

MatPhyConfig;

% check for type of CCCH_MODE_REQ
switch phyconnect.Data(121)
    case CCCH_MODE_NONE
        fprintf('CCCH_MODE type: CCCH_MODE_NONE\n');
        % TODO: set some variable
        send_CCCH_MODE_CONF(phyconnect,CCCH_MODE_NONE,tpu);
    case CCCH_MODE_NON_COMBINED
        fprintf('CCCH_MODE type: CCCH_MODE_NON_COMBINED\n');
        % TODO: set some variable
        send_CCCH_MODE_CONF(phyconnect,CCCH_MODE_NON_COMBINED,tpu);
    case CCCH_MODE_COMBINED
        fprintf('CCCH_MODE type: CCCH_MODE_COMBINED\n');
        % TODO: set some variable
        send_CCCH_MODE_CONF(phyconnect,CCCH_MODE_COMBINED,tpu);
    otherwise
...
function send_CCCH_MODE_CONF(phyconnect,type)

constants;

% wait for phyconnect to be ready
while 1
    if phyconnect.Data(3) == 0
        break;
    end
end
...
% send message
tpu.display;
phyconnect.Data(17) = 0; % msg done flag
fprintf('Sending CCCH_MODE_CONF\n');
phyconnect.Data(19) = CCCH_MODE_CONF;
phyconnect.Data(21) = type;
